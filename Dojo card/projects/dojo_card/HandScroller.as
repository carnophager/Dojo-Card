package projects.dojo_card
{
	...
	public class HandScroller extends Sprite
	{
		...
		private var _x_position		:Number;
		...
		private function createHand():void 
		{
			...
			for (i = 0; i < length; i++) 
			{
				if ( i == length - 1 )
				{
					addBitmap('hand_end_0');
					addBitmap('hand_end_1');
					addBitmap('hand_end_2');
				} else {
					addBitmap('hand_loop_0');
					addBitmap('hand_loop_1');
				}
			}
		}
		
		private function addBitmap(name_:String):void 
		{
			var bmp:Bmp = new Bmp(_graphic, name_);
			
			bmp.x = _x_position;
			_x_position += bmp.width;
